...
public static class EyreUtility
{
    ...
    /// <returns></returns>
    public static Vector3[] GenerateCirclePoints(Vector3 center, int pointCount,float radius = 2f)
    {
        float splitAngle = Mathf.PI * 2 / pointCount;
        Vector3[] points = new Vector3[pointCount];
        for (int i = 0; i < pointCount; i++)
        {
            points[i] = new Vector3(radius * Mathf.Cos(splitAngle * (i + 1)), radius * Mathf.Sin(splitAngle * (i + 1)), 0) + center;
        }
        return points;
    }
